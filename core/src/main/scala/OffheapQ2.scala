...
@data class TupleString(
  ...
  def iterator = __1.iterator ++ __2.iterator ++ __3.iterator ++ 
                 __4.iterator ++ __5.iterator ++ __6.iterator ++ 
                 __7.iterator ++ __8.iterator ++ __9.iterator ++ 
                 __10.iterator

  def endsWith(that: TupleString): Boolean = endsWith(that.iterator)
  def endsWith(that: OptimalString): Boolean = endsWith(that.data.iterator)
  def endsWith(thatIter: Iterator[Byte]): Boolean = {
    val i = this.iterator.drop(length - thatIter.length)
    val j = thatIter
  ...
  def ===(that: OptimalString): Boolean = compare(that) == 0
  def =!=(that: TupleString): Boolean = compare(that) != 0
  def =!=(that: OptimalString): Boolean = compare(that) != 0
  def compare(that: TupleString): Int = compare(that.iterator)
  def compare(that: OptimalString): Int = compare(that.data.iterator)
  def compare(thatIter: Iterator[Byte]): Int = 
    if (length == thatIter.length) {
      val i = this.iterator
      val j = thatIter
      while(i.hasNext && j.hasNext)
        if(i.next != j.next)
...
object TupleString {
  def apply(data: scala.Array[Byte])(implicit alloc: Allocator): TupleString = {
    ...
    val __1 = if(remaining > 21)
                ...
                val rem = remaining.toByte
                remaining = 0
                String21(data, 0 * 21, rem.toByte)
              }
    remaining -= 21
    ...
    val __10 = if(remaining > 21)
                ...
                val rem = remaining.toByte
                remaining = 0
                String21(data, 9 * 21, rem)
              }
    TupleString(__1, __2, __3, __4, __5, __6, __7, __8, __9, __10, 
      __1.len + __2.len + __3.len + 
      __4.len + __5.len + __6.len + 
      __7.len + __8.len + __9.len + 
      __10.len)
  }
